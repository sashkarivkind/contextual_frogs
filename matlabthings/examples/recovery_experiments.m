...
for i_paradigm = 1:length(paradigms)
    ...
    for i_subj=1:8
        ...
        perturbations_ = measuredData.TrialData.FieldConstants;
        model.perturbations = preprocPert(perturbations_);
...
%%
function o = preprocPert(pert)
   o = pert(:,1);
   chndx = abs(o) > 29; %it's 30 for a channel trial
   o = o/0.15; %strength of a non-channel field
   o(chndx) = NaN;
end

%%
if model.plot_responsibilities
figure ; 
for ii = 1:16
subplot(4,4,ii)
plot(results{ii}.plots.known_context_responsibilities);
hold on;
plot(results{ii}.plots.novel_context_responsibility);

end
end

%%
if model.plot_predicted_probabilities
figure ; 
for ii = 1:16
subplot(4,4,ii)
plot(results{ii}.plots.predicted_probabilities);

end
end

%%
if model.plot_state_given_context
figure ; 
for ii = 1:16
subplot(4,4,ii)
for jj=1:size(results{ii}.plots.state_given_context,3)
imagesc(results{ii}.plots.state_given_context(:,:,jj));
end
end
end
...
for i_paradigm = 1:length(paradigms)
    ...
    for i_subj=1:8
        ...
        subplot(4,4,cnt)
        measuredData = load(['/homes/ar2342/frogs_project/data/COIN_data/',paradigm,'_recovery_participant',num2str(i_subj),'.mat']);
        plot(measuredData.TrialData.Adaptation,'.');
        hold on;
        plot(results{cnt}.plots.average_state_feedback);
        errs = measuredData.TrialData.Adaptation - results{cnt}.plots.average_state_feedback;
        tail_errs(cnt,:) = errs(end-100:end);
    end
end
