...
runname = '/homes/ar2342/frogs_project/COIN/results/all_r10_p1000_'; % TSTantero_10runsExact'
if ~exist(runname,'dir')
    mkdir(runname);
end

% your stimuli definition goes here...
Pplus=1; Pminus=-1; P0=0; Pchannel = NaN;
TaN=150; TaB=120;
TaN=160; TaB=115;
lng_TaN = 369+TaN;
TKitagoN=40; TKitagoA=80; TKitagoC=200; 
...
stimuli = struct( ...
  'Kitago', [ P0*ones(1,TKitagoN), Pplus*ones(1,TKitagoA),    Pchannel*ones(1,TKitagoC),   Pplus*ones(1,TKitagoA) ], ...
  'KitagoX2', [ P0*ones(1,2*TKitagoN), Pplus*ones(1,2*TKitagoA),    Pchannel*ones(1,2*TKitagoC),   Pplus*ones(1,2*TKitagoA) ],...
  'KitagoX0p5',[ P0*ones(1,floor(TKitagoN/2)), Pplus*ones(1,floor(TKitagoA/2)),    Pchannel*ones(1,floor(TKitagoC/2)),   Pplus*ones(1,floor(TKitagoA/2)) ] ...
);

% stimuli = struct( ...
%   'lng_AB0', [ P0*ones(1,lng_TaN),           Pminus*ones(1,TaB) ], ...
%   'lng_AB1', [ P0*ones(1,lng_TaN-13),   Pplus*ones(1,13),  Pminus*ones(1,TaB) ], ...
%   'lng_AB2', [ P0*ones(1,lng_TaN-41),   Pplus*ones(1,41),  Pminus*ones(1,TaB) ], ...
%   'lng_AB3', [ P0*ones(1,lng_TaN-112),  Pplus*ones(1,112),  Pminus*ones(1,TaB) ], ...
%   'lng_AB4', [ P0*ones(1,lng_TaN-230),  Pplus*ones(1,230),  Pminus*ones(1,TaB) ], ...
%   'lng_AB5', [ P0*ones(1,lng_TaN-369),  Pplus*ones(1,369),  Pminus*ones(1,TaB) ]  ...
% );

% stimuli = struct(...
%     'frogs', [P0*ones(1,136), Pplus*(1:240)/240,P0*ones(1,96)],...
%     'anti_frogs', [P0*ones(1,136), Pplus*ones(1,240),P0*ones(1,96)]...  
%     );


% stimuli = struct(...
%     'toy__savings', [P0*ones(1,136), Pplus*ones(1,240),P0*ones(1,240), Pplus*ones(1,240)]...  
%     );
% build subject list
subject_alias = {};
...
% for i=1:1, subject_alias{end+1} = ['A'];  end
...
%         model = assign_coin_params(model, subj, paramsTable);
        ...
        model.runs = 10;
        model.infer_bias = false;
        model.particles=1000;
